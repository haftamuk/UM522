...
if (parts['protocol_id'] === '01' && parts['data'].length >= 16) {
  // Convert hex IMEI to decimal string
  const imeiHex = parts['data'].substr(0, 16);
  let imei = '';
  
  // Convert each byte (2 hex chars) to decimal
  for (let i = 0; i < 15; i += 2) {
    // Take 2 hex characters (1 byte)
    const hexByte = imeiHex.substr(i, 2);
    const decimal = parseInt(hexByte, 16);
    
    // Convert to 2-digit decimal string
    if (i < 14) {
      // First 7 bytes = 14 hex chars = 14 decimal digits
      imei += decimal.toString().padStart(2, '0');
    } else {
      // Last byte = 1 decimal digit (15th digit)
      imei += Math.floor(decimal / 10).toString();
    }
  }
  
  parts['device_id'] = imei;
  console.log(`Extracted IMEI from hex ${imeiHex}: ${imei}`);
} else {
  parts['device_id'] = '';
}
